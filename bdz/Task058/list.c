...
Node* mergeTwoLists(Node* head1, Node* head2) {
    ...
    Node* merged = NULL; // начало списка
    Node* tail = NULL;   // узел, через котоый формируем весь остальной список, а не просто конец
    Node* current = NULL;
    ...
    while(head1 != NULL && head2 != NULL) {
        ...
        // если в головном узле ничего нет, записываем
        if(merged == NULL) {
            merged = current;
            tail = current;
        }
        // сдвигаемся и записыаем (merged всё еще указывает на начало)
        else {
            tail->next = current;
            tail = tail->next;
        }
